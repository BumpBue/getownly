...
- **ข้อจำกัดที่รู้แล้วและยังไม่ได้แก้: ภาพหน้าปกไม่ขึ้นเมื่อรันด้วย `--profile full`**
  ตัวไฟล์ไม่มีปัญหา (เปิด presigned URL ตรงๆ ในเบราว์เซอร์ได้ครบ) แต่ `next/image` ทำ optimize
  **ฝั่งเซิร์ฟเวอร์** คือ container `web` ไปดึง `http://localhost:9000/...` ซึ่งใน container นั้น `localhost`
  คือตัวมันเอง ไม่ใช่ MinIO (พิสูจน์แล้ว: ใน `web` ยิง `localhost:9000` ได้ ECONNREFUSED ส่วน `minio:9000` ได้ 403)
  ทางแก้ที่ตรงกับของเดิมที่สุดคือให้ปกใช้ `<img>` เหมือนที่ QR และสลิปทำอยู่แล้ว (URL หมดอายุ + ไม่รู้ขนาดล่วงหน้า)
  **บนเครื่องพัฒนาไม่เจอปัญหานี้** เพราะ optimizer รันบน host ที่ `localhost:9000` คือ MinIO จริง
