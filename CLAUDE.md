...
- **`signingClient` ต้องถูก "บอก" region ไม่ใช่ปล่อยให้ไปถาม** — minio-js ที่ไม่ได้ตั้ง `region` จะยิงถาม
  ...
  ผลคือ `POST /uploads/presign` ตอบ 500 (`ECONNREFUSED 127.0.0.1:9000`) ทุกครั้งใน Docker
...
- **ห้ามใช้ `next/image` กับไฟล์จาก MinIO ทุกกรณี — ใช้ `<img>` เท่านั้น**
  ...
  ถอดรหัสเป็นภาพได้) เพราะ container `web` ไปดึง `http://localhost:9000/...` ซึ่งใน container นั้น
  `localhost` คือตัวมันเอง (พิสูจน์แล้ว: ใน `web` ยิง `localhost:9000` ได้ ECONNREFUSED · `minio:9000` ได้ 403)
  · **บนเครื่องพัฒนาไม่มีทางเจอ** เพราะ optimizer รันบน host ที่ `localhost:9000` คือ MinIO จริง
