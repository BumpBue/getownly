...
**พอร์ตมาตรฐาน:** frontend `3000` · backend `4000` · postgres `5433` (พอร์ตบนเครื่อง — ภายใน container ยังเป็น `5432` ที่ตั้ง 5433 เพราะเครื่องพัฒนามักมี PostgreSQL ของตัวเองครอง 5432 อยู่แล้ว) · **MinIO API `9100` / Console `9101`** (พอร์ตบนเครื่อง — ภายใน container ยังเป็น `9000`/`9001` เหตุผลอยู่ในหัวข้อ "พอร์ตของ MinIO" ด้านล่าง) · Mailhog SMTP `1025` / UI `8025`
...
**สถานะ: เฟส 1–8 ทำครบ · ข้อกำหนด ทก.01 ทั้ง 37 ข้อ (A1–D9) ทำครบและมีเอกสารเทียบใน `SCOPE_COMPLIANCE.md`**

ทำเพิ่มหลังจากนั้น (นอกผัง 8 เฟส): หน้าจอสร้างแบบทดสอบของผู้สอน · รายชื่อผู้เรียนรายคอร์ส (A9) ·
ตารางส่วนแบ่งรายธุรกรรม (A10) · **ระบบถอนเงินของผู้สอนพร้อมบัญชีธนาคาร** (ส่วนขยายนอก ทก.01) ·
ดูตัวอย่างบทเรียนฟรี · Docker profile `full` · `test:responsive` และ `test:demo-flow` ด้วยเบราว์เซอร์จริง

**ยังไม่มี (ทุกข้ออยู่นอก ทก.01 และขาดทั้ง model และหน้าจอ ไม่ใช่ของค้างครึ่งทาง):**
ตะกร้า/checkout และ `Order`/`OrderItem` · รีวิวและให้ดาว · ใบประกาศนียบัตร · การแจ้งเตือน ·
`PlatformSetting` · `AuditLog` · การส่งออก CSV · การยืนยันอีเมล · อีเมลแจ้งผลตรวจสลิป ·
โปรไฟล์ผู้สอนสาธารณะ

> ⚠️ ย่อหน้านี้เคยล้าสมัยจนบอกสถานะผิดสามข้อพร้อมกัน (บอกว่าหน้าจอสร้างข้อสอบ การถอนเงิน
...
### เทสต์ตกหนึ่งข้อโดยไม่ทราบสาเหตุ — ยังหาไม่เจอ

**สถานะ: ยังไม่ทราบสาเหตุ ไม่ได้แก้อะไร** บันทึกไว้เพื่อให้ครั้งหน้าเริ่มจากตรงนี้ ไม่ใช่เริ่มใหม่

**สิ่งที่เกิดขึ้น** — ชุดเทสต์ backend ตก **1 ข้อจาก 486** หนึ่งครั้ง ขณะที่กำลังรัน
`docker compose build` ขนานอยู่ · **ไม่ได้จับชื่อเทสต์และข้อความ error ไว้** ข้อความหายไปกับ
scrollback · รันซ้ำทันที 3 รอบผ่านหมด

**สิ่งที่ทำไปเพื่อตามหา แล้วยังหาไม่เจอ**

- **10 รอบบนเครื่องว่าง = 4,860 ข้อ ผ่านหมด ไม่ตกสักข้อ** (รอบละ 213–273 วินาที)
  → **ไม่มีหลักฐานว่ามีเทสต์ตัวใดไม่เสถียรจริง**
- ตัดผู้ต้องสงสัยออกด้วยการอ่านโค้ด: ข้อมูลรั่วข้ามเทสต์ (มี `fileParallelism: false`
  และ `resetDatabase()` TRUNCATE ทุกตาราง) · พึ่งนาฬิกาปัจจุบัน (เทสต์ D3 ใช้ระยะห่าง ±1 ชม.
  จากเกณฑ์ 24 ชม.) · ลำดับผลลัพธ์ไม่แน่นอน (ที่ assert ลำดับจริงมี `id` เป็นตัวตัดสินรองแล้ว)
- **สมมติฐาน `MONEY_TRANSACTION_OPTIONS.maxWait` ถูกหักล้างแล้ว** เดาไว้ว่าธุรกรรมท้ายคิว
  จะหมดเวลารอ row lock แล้วโยน error ของ Prisma แทน business exception ที่ assertion คาดไว้
  · ทดสอบใต้โหลดหนักจริงแล้ว **อาการที่ได้คือ `Hook timed out in 30000ms` ที่ `beforeEach`**
  ตรงบรรทัด `prisma.user.create` / `prisma.category.create` ใน `factories.ts` **ไม่ใช่ lock timeout**
  · และ**เหยื่อเป็นใครก็ได้** รวมเทสต์ที่ไม่มี concurrency เลย เช่นข้อที่แค่คำนวณการปัดเศษ
  รอบที่หนักที่สุดตกทั้ง 44 ข้อในไฟล์ · มี `40P01 deadlock detected` ระหว่าง `TRUNCATE`
  ของสองไฟล์ที่ทับกันด้วย ซึ่งเป็นอาการของเครื่องอดตาย ไม่ใช่ของโค้ดที่กำลังทดสอบ
- **การทดลองใต้โหลดพิสูจน์น้อยกว่าที่ตั้งใจ** มันบอกแค่ว่า "ถ้าอดเครื่องมากพอ setup จะไม่ทัน"
  ซึ่งจริงกับชุดเทสต์ทุกชุด · และ**เทียบกับครั้งแรกไม่ได้ เพราะไม่มี error ของครั้งแรกเก็บไว้**

**ทำไมถึงไม่แก้อะไร** — หลักฐานไม่พอจะบอกได้ว่าจะแก้ตรงไหน · การขยับ `maxWait` ของโค้ดจริง
เพื่อให้เทสต์ผ่านคือการเปลี่ยนพฤติกรรม production เพื่อความสะดวกของเทสต์ · การแก้เทสต์
ตามสมมติฐานที่หักล้างไปแล้วก็ไม่มีเหตุผลรองรับ

**ครั้งหน้าให้เริ่มจากตรงนี้** — `backend/vitest.config.ts` เปิด JSON reporter ไว้แล้ว
ทุกครั้งที่รัน `pnpm test` จะเขียน `backend/test-results/last-run.json` ทับของเดิม
ในนั้นมี `fullName` `status` `duration` และ `failureMessages` เต็มของทุกข้อ
**ถ้าตกอีก ให้เปิดไฟล์นั้นทันทีก่อนรันซ้ำ** (รันซ้ำจะทับไฟล์)
`test-results/` อยู่ใน `.gitignore` แล้ว

### พอร์ตของ MinIO: `9100`/`9101` บนเครื่อง ไม่ใช่ `9000`/`9001`

**อาการที่ทำให้ต้องย้าย** — อยู่ๆ `docker compose up` ก็ขึ้นว่า

```
Error response from daemon: ports are not available:
exposing port TCP 0.0.0.0:9001 -> ...
bind: An attempt was made to access a socket in a way forbidden by its access permissions
```

**ไม่ได้แปลว่ามีโปรแกรมอื่นใช้พอร์ตอยู่** — `netstat` ไม่เห็นใครฟังเลย แต่ Windows
**จองช่วงพอร์ตไว้เอง** ตรวจได้ด้วย

```powershell
netsh interface ipv4 show excludedportrange protocol=tcp
```

ตอนที่เจอ ช่วงที่ถูกจองคือ **8916–9015** ซึ่งกลืนทั้ง 9000 และ 9001 พร้อมกัน
MinIO จึงสตาร์ตไม่ขึ้นเลย และการอัปโหลดทั้งระบบใช้ไม่ได้

- **คนจองคือ Hyper-V NAT (WinNAT)** ซึ่งกันช่วงพอร์ตไว้ให้ตัวเองแบบไดนามิกหลังรีสตาร์ต
  หรือหลัง WSL/Docker Desktop เริ่มใหม่ · **ช่วงที่จองเปลี่ยนไปได้เรื่อยๆ**
  ยืนยันแล้วในเครื่องเดียวกัน: ตรวจครั้งแรกจอง 8916–9015 · อีกยี่สิบนาทีเหลือแค่ 50000–50059
  **จึงเป็นบั๊กที่หายเองและกลับมาเองโดยไม่มีใครแตะอะไร ซึ่งเป็นแบบที่แย่ที่สุดสำหรับวันสอบ**
- **ย้ายเฉพาะพอร์ตฝั่งเครื่อง ไม่ได้ย้ายพอร์ตใน container** — ใน compose network
  MinIO ยังฟัง `9000`/`9001` เหมือนเดิม `minio:9000` ทุกที่จึงไม่ต้องแก้
  ที่เปลี่ยนคือ `ports:` ที่ publish ออกมา (`9100:9000`, `9101:9001`)
  และ `MINIO_PUBLIC_PORT` ที่ใช้เซ็น presigned URL ให้เบราว์เซอร์
  · บนเครื่องพัฒนา (`pnpm dev`) backend อยู่บน host จึงต้องใช้ `MINIO_PORT=9100` ด้วย
- **ย้ายแล้วไม่ได้แปลว่าปลอดภัยถาวร** WinNAT เลือกช่วงเองและวันหนึ่งอาจกิน 9100 ได้เหมือนกัน
  ที่ได้จริงคือ**ลดโอกาสชน** เพราะ 9000 เป็นพอร์ตยอดนิยม (MinIO, PHP-FPM, SonarQube)
  จึงชนกับทั้ง WinNAT และซอฟต์แวร์อื่น
  · **วิธีกันขาดจริงต้องใช้สิทธิ์ผู้ดูแลครั้งเดียว** — จองพอร์ตดักไว้ก่อนที่ Windows จะเอาไป:
  `netsh int ipv4 add excludedportrange tcp 9100 2 persistent`
  · **วิธีแก้เฉพาะหน้าเมื่อเจออาการ**: `net stop winnat` แล้ว `net start winnat` (PowerShell แบบผู้ดูแล)
- **อย่าเปลี่ยนพอร์ตกลับไปเป็น 9000 เพราะ "เป็นมาตรฐาน"** ความเป็นมาตรฐานคือสาเหตุที่มันชน

### ถอดกระดิ่งแจ้งเตือนออก (และเกณฑ์ว่า affordance แบบไหนถือว่าหลอก)

**`NotificationBell` ถูกลบทิ้งแล้ว ไม่ได้ถูกแทนที่ด้วยระบบแจ้งเตือนจริง**
มันเป็นกระดิ่งบน header ของผู้เรียนทุกหน้า กดแล้วเปิดแผงที่เขียนว่า "จะเปิดใช้งานในเวอร์ชันถัดไป"
ไฟล์เขียนกำกับตัวเองไว้ตรงๆ ว่าเป็น placeholder และจงใจไม่ปลอมตัวเลขค้าง ซึ่งซื่อสัตย์
แต่มันคือ**สิ่งเดียวในระบบที่สัญญาว่ามีของแล้วไม่มี** และอยู่ในตำแหน่งที่กรรมการจะกดแน่นอน

- **ทก.01 ไม่ได้กำหนดเรื่องการแจ้งเตือน** การสร้างระบบขึ้นมารองรับกระดิ่งจึงเป็นการขยายขอบเขต
  ที่เอกสารไม่ได้ขอ ส่วนการปล่อยกระดิ่งไว้คือการโฆษณาของที่ไม่มี · ถอดออกจึงเป็นทางที่เหลือ
- **ถ้าจะทำจริงในอนาคตต้องมีครบสามอย่าง** ไม่ใช่แค่คืนกระดิ่ง:
  model `Notification` (userId, type, title, body, linkUrl, readAt) ตามที่ PLAN.md หัวข้อ 1.6 วางไว้ ·
  endpoint อ่าน/ทำเครื่องหมายว่าอ่านแล้ว · หน้า `/notifications` และตัวนับบนกระดิ่ง
  · และต้องมีคนเขียน Notification จริงตอนเกิดเหตุ (ตอบคำถาม อนุมัติสลิป อนุมัติคอร์ส อนุมัติถอนเงิน)
  ไม่งั้นก็ได้กระดิ่งที่ว่างเปล่าเหมือนเดิม
- **หลังถอดแล้ว header ไม่มีช่องโหว่** ทั้งสองจุดที่มันเคยอยู่เป็น flex ที่มี `gap` อยู่แล้ว
  เดสก์ท็อปเหลือ `WalletPill` + `UserMenu` · แถบมือถือที่เคยเป็น `flex` ครอบสองตัว
  ยุบเหลือ `WalletPill` ตัวเดียว ไม่ต้องมี wrapper · ตรวจ 375/768/1440 ด้วย `test:responsive` แล้ว

**เกณฑ์ว่า affordance แบบไหนถือว่า "หลอก" — ตัดสินไว้แล้ว ไม่ต้องถกซ้ำ**

ตอนไล่ตรวจทั้งระบบเจอ `<tr>` ที่เปลี่ยนสีตอน hover โดยไม่มีคอนโทรลในแถวเลย **4 จุด**
(คอร์สขายดีและผู้สอนยอดนิยมใน `ReportsView` · `EarningsTable` · `WalletView`)
**ตัดสินว่าไม่ต้องแก้** เพราะแถบไฮไลต์ทั้งแถวเป็นธรรมเนียมของตารางที่ช่วยกวาดสายตาข้ามคอลัมน์
ต่างจากเคสปุ่มดูตัวอย่างตรงที่นั่นมี **ป้าย "ดูตัวอย่างได้" + ไอคอนเล่นที่เปลี่ยนสีตาม hover**
ประกอบกัน ซึ่งรวมแล้วอ่านว่า "กดสิ" · **hover เฉยๆ บนแถวตาราง = ตัวช่วยอ่าน · hover + ป้าย
หรือไอคอนที่สื่อการกระทำ = คำสัญญา** ถ้าเจอกรณีใหม่ ให้ตัดสินด้วยเส้นนี้

### ดูตัวอย่างบทเรียนฟรีบนหน้าคอร์ส

**ที่มา: ผู้ใช้แจ้งว่า "ปุ่มดูตัวอย่างกดแล้วไม่ทำงาน" — ความจริงคือไม่เคยมีปุ่มนั้นอยู่เลย**
มีแค่ป้าย `<Badge>` "ดูตัวอย่างได้" กับไอคอน `PlayCircle` · หลังบ้านทำเสร็จตั้งแต่เฟส 3
(`canReadLessonContent()` ปล่อย `isPreview` ผ่านอยู่แล้ว) และ `PLAN.md:333` เขียนไว้ว่าหน้านี้ต้องมี
แต่ไม่เคยต่อหน้าจอ · **ทก.01 ไม่ได้กำหนดข้อนี้** จึงไม่กระทบการ audit

- **ต้นเหตุที่ทำให้เข้าใจผิดคือ affordance ที่หลอก** — ทุกแถวมี `hover:bg-background`
  และไอคอนเปลี่ยนสีตอน hover ทั้งที่ไม่มีแถวไหนกดได้ · ตอนนี้เหลือเฉพาะแถว preview
  ที่เป็น `<button>`/`<Link>` จริง แถวอื่นเป็น markup เฉยๆ ไม่มี hover
  **บทเรียน: อย่าใส่ hover ให้สิ่งที่กดไม่ได้**
- **แยก "เข้าถึงไม่ได้" ออกจาก "เล่นไม่ได้"** — probe ด้วย `fetch` ก่อน mount `<video>`
  เพื่อให้บอกสาเหตุเป็นภาษาไทยได้ (404 = ยังไม่อัปโหลดวิดีโอ · 403 = ไม่ใช่บทตัวอย่าง ·
  401 = เซสชันหมดอายุ) เพราะ `<video onError>` บอกไม่ได้ว่าเพราะอะไร
  · ถ้า probe ผ่านแต่ decode ไม่ได้ **ยังคง player ไว้แล้วขึ้น alert ข้างล่าง** ตามแบบ `LessonPlayer`
  ไม่ใช่ถอด player ทิ้ง — เจอจากเทสต์ตัวเอง: ตอนแรกเขียนให้ `onError` ไปตั้ง `failure`
  ทำให้ fixture MP4 ที่ decode ไม่ได้ ลบ player ทิ้งทั้งที่สตรีมสำเร็จ (206)
- **ปิด dialog ต้อง `pause()` + `removeAttribute('src')` + `load()`** ไม่งั้นวิดีโอเล่นต่อเบื้องหลัง
  และ request ยังค้างอยู่ที่ API
- **guest ได้ `<Link>` ไป `/login?next=...` ไม่ใช่ปุ่ม** เพราะ `/lessons/:id/stream` ไม่ใช่ `@Public()`
  · **จงใจไม่แตะสิทธิ์ฝั่ง backend เลยแม้แต่บรรทัดเดียว** (ข้อ D1)
  · หน้ารู้ว่าใครล็อกอินอยู่จาก `balance !== null` ซึ่งเป็นสัญญาณเดียวที่หน้านี้มี (แบบเดียวกับ `PurchasePanel`)
- **`createHarness({ fakeStorage: true })` เป็นของใหม่** ก่อนหน้านี้ e2e ใช้ MinIO จริงเสมอ
  เทสต์ที่ต้องพิสูจน์ว่า "ไบต์ไหนถูกส่งกลับมา" ต้องคุมเนื้อไฟล์ได้ จึง override `StorageService`
  ด้วย `FakeStorage` ตามเหตุผลเดิมของเฟส 3 (ฐานข้อมูลต้องจริงเพราะ lock/transaction คือสิ่งที่ทดสอบ
  ส่วน object storage ตอบแค่ "key ไหน/ไบต์ไหน")
- **เทสต์ที่สำคัญที่สุดคือ "preview ต้องไม่กลายเป็นช่องเลี่ยงการซื้อ"** —
  `lesson-preview.e2e.spec.ts` ยิง endpoint ตรงด้วย session จริงกับบทที่ไม่ใช่ preview
  ในคอร์สเดียวกัน ต้องได้ 403 ทุกรูปแบบของ Range · และยืนยันว่า payload ของ `GET /courses/:id`
  ไม่มีสตริง `video/` หลุดออกไป

### ระบบถอนเงินของผู้สอน (ส่วนขยาย **นอก** ทก.01)

**ข้อนี้ไม่ได้อยู่ในเอกสาร** ผู้ใช้ตัดสินใจเพิ่มเองหลัง A1–D9 ครบแล้ว
มีหัวข้อ 13 ใน `SCOPE_COMPLIANCE.md` อธิบายให้กรรมการเห็นว่าเป็นการเพิ่มโดยตั้งใจ ไม่ใช่ scope creep

- **เงินออกจากกระเป๋าตอน "ยื่นคำขอ" ไม่ใช่ตอน "อนุมัติ"** เลือกแบบนี้ทั้งที่แพงกว่า (ต้องมีบัญชีพัก + ต้องกลับรายการ)
  เพราะแบบที่ถูกกว่า (กันยอดไว้เฉยๆ) ทำให้เงินก้อนเดียวกันถูกทั้งขอถอนและเอาไปซื้อคอร์สได้
  ซึ่งอธิบายไม่ได้ · ผลที่ได้คือ **`Account.balance` = ยอดที่ใช้ได้จริงเสมอ ไม่มีแนวคิดที่สอง**
  (มีเทสต์ `(ญ)` ที่ยื่นขอถอนแล้วพยายามซื้อคอร์สด้วยเงินก้อนเดียวกันแล้วต้องถูกปฏิเสธ)
- **`AccountKind.PAYOUT_PAYABLE` เป็นค่าที่สี่** — ก่อนเพิ่ม ไล่หาที่ enumerate ไว้ **เจอ 7 จุด**:
  `packages/shared/src/enums.ts` · ป้ายไทยใน `messages/admin.ts` · งบทดลองใน `reports.service.ts` ·
  `prisma/seed.ts` · `prisma/bootstrap-admin.ts` · `test/factories.ts` (`createSystemAccounts`) ·
  `reports.service.spec.ts` ที่ assert รายชื่อ kind ในงบทดลอง
  **`prisma/demo-seed.ts` ไม่ต้องแก้** เพราะมันหาแค่สองบัญชีที่ตัวเองใช้ ไม่ได้ไล่ทั้ง enum
- **`ALTER TYPE ... ADD VALUE` กับ `INSERT` ที่ใช้ค่านั้น อยู่ migration เดียวกันไม่ได้**
  Postgres ห้ามใช้ค่า enum ที่เพิ่งเพิ่มภายใน transaction เดียวกัน และ Prisma รัน migration ละ transaction
  จึงต้องแยกเป็น `20260909100000` (ตาราง+enum) และ `20260909100100` (สร้างแถวบัญชี)
  · **สร้างแถวบัญชีใน migration ไม่ใช่ใน seed** เพื่อให้ฐานข้อมูลที่มีข้อมูลจริงอยู่แล้วได้บัญชีนี้ด้วย
- **หนึ่งคำขอสร้างได้ถึง 3 ธุรกรรม แยกด้วย `referenceType`** (`PayoutRequest` ตอนยื่น ·
  `PayoutSettlement` ตอนอนุมัติ · `PayoutReversal` ตอนปฏิเสธ/ยกเลิก) ทั้งสามเป็น `TxType.PAYOUT` เหมือนกัน
  **`referenceType` เป็นตัวเดียวที่แยกได้ว่า "โอนออกจริงแล้ว" กับ "แค่ขอมา"** และรายงานพึ่งค่านี้โดยตรง
  ห้ามเปลี่ยนสตริงสามตัวนี้โดยไม่แก้ SQL ของ `topInstructors` พร้อมกัน
- **`outstanding` นับเฉพาะที่โอนออกจริง ไม่นับคำขอที่ยังค้าง** — เป็นการเลือกนิยาม ไม่ใช่ผลพลอยได้
  ถ้านับตามกระเป๋า ตัวเลขจะลดตอนยื่นแล้วเด้งกลับตอนถูกปฏิเสธ ซึ่งอ่านรายงานย้อนหลังไม่ได้ความ
  **ผลคือ `outstanding` ไม่เท่ากับยอดในกระเป๋าผู้สอนอีกต่อไป** และมีข้อความอธิบายบนหน้ารายงาน
- **`instructorPayable` ในรายงาน ADMIN ไม่เคยหมายถึงยอดคงค้างเลย** มันคือรายได้ที่เกิด**ในช่วงที่เลือก**
  (`WHERE lt.type='PURCHASE'` + ช่วงเวลา) ป้ายเดิมเรียกผิดมาตลอด แก้เป็น "รายได้ผู้สอน (สะสมในช่วง)"
  **ค่าไม่ขยับแม้แต่สตางค์เดียว** เพราะ `ส่วนแบ่งแพลตฟอร์ม + รายได้ผู้สอน = ยอดขายรวม` ต้องจริงทุกช่วงเวลา
- **มีเทสต์ล็อกค่ารายงานเดิมไว้ทั้งชุด เขียนตัวเลขด้วยมือ** (`reports.service.spec.ts`
  "reports exactly the same figures as before payouts existed") · **พิสูจน์แล้วโดยเอา query เก่ากลับมาวางแล้วรันซ้ำ**
  ผ่านทั้งสองเวอร์ชัน ไม่ใช่แค่ผ่านกับโค้ดใหม่
- **บัญชีธนาคารอยู่ตารางแยก `InstructorBankAccount` ไม่ใช่คอลัมน์บน `User`** เพราะ `User` ถูก select
  หลายที่ทั่วระบบ · `PayoutRequest` snapshot ข้อมูลบัญชีไว้ตอนยื่น (แก้บัญชีทีหลังไม่เขียนอดีตใหม่)
  · **`maskAccountNumber()` ใน `src/common/bank-account.ts` เป็นที่เดียวที่ปิดบัง**
  · **เลขเต็มออกได้ 2 ทางเท่านั้น**: ฟอร์มของเจ้าของ และรายละเอียดคำขอที่ ADMIN เปิด
  **คิวของ ADMIN ปิดบังด้วย** — เจอบั๊กนี้จากเทสต์ที่ตัวเองเขียน รอบแรก listing ส่งเลขเต็มมาทั้งหน้า
  แก้ด้วยการแยก `AdminPayoutListItemDto` (masked) ออกจาก `AdminPayoutRequestDto` (เต็ม)
  และให้ dialog ไปดึงรายการเองตอนเปิด
- **เทสต์กวาดทั้ง response ด้วย regex เลขบัญชี ไม่ได้ตรวจเฉพาะฟิลด์ที่นึกออก** แนวเดียวกับ
  `quiz-scoring.spec.ts` ที่ไล่อ่านซอร์ส · เทสต์ e2e ให้ผู้สอนแต่ละคนมีเลขบัญชีคนละเลข
  ไม่งั้น "เจอเลขของ BANK ใน response ของคนอื่น" จะไม่ได้แปลว่ารั่วจริง
- **`PAYOUT_MIN_AMOUNT_BAHT = 500` อยู่ใน `limits.ts` ใต้หัวข้อ `--- extensions beyond ทก.01 ---`**
  ไฟล์นั้นเขียนหัวไว้ว่าเป็นตัวเลขจากเอกสาร จึงต้องติดป้ายให้ชัดว่าอันนี้ไม่ใช่
  · `scope-limits.spec.ts` แยก describe block ออกมาต่างหากด้วยเหตุผลเดียวกัน
- **บังคับซ้ำที่ฐานข้อมูล 2 ชั้นที่ Prisma มองไม่เห็น** — CHECK `amount >= 500` และ
  **partial unique index `WHERE status = 'PENDING'`** (คำขอค้างได้ 1 ใบ)
  ทั้งคู่ทำให้ `prisma migrate dev` คิดว่า drift เหมือน CHECK constraint ของ ทก.01 ที่มีอยู่ก่อน
  **ห้ามรัน `migrate dev` กับฐานที่มีของพวกนี้โดยไม่อ่านหัวข้อ schema drift ก่อน**
- **ธนาคารเป็น dropdown เก็บรหัส ธปท. 3 หลัก ไม่ใช่ free text** รายการ 16 แห่งอยู่ที่
  `packages/shared/src/banks.ts` ที่เดียว · เก็บรหัสเพราะชื่อบริษัทเปลี่ยนได้แต่รหัสไม่เปลี่ยน
  (ttb ยังเป็น 011 ของ TMB เดิม · 065 ของธนชาตเลิกใช้แล้ว)
  · **`InstructorBankAccount` เก็บรหัสอย่างเดียว** (ชื่อ derive ได้ เก็บคู่ = สำเนาที่สองที่เพี้ยนตามกันไม่ทัน)
  · **`PayoutRequest` เก็บทั้งรหัสและชื่อ** เพราะเป็นประวัติ ต้องอ่านออกแม้รหัสหลุดจากรายการ
  `bankDisplayName(code, fallback)` เป็นตัวตัดสินว่าจะแสดงชื่อจากรายการหรือจาก snapshot
- **`logoFile` เก็บชื่อไฟล์เต็มพร้อมนามสกุล ไม่ประกอบจาก `code + '.svg'`**
  เพราะไฟล์จริงมีทั้ง `.gif` (13 ไฟล์) และ `.jpg` (2 ไฟล์) การประกอบ path จะ 404 เงียบๆ กับสองไฟล์นั้น
  · **071 ไทยเครดิตยังไม่มีโลโก้ → `logoFile: null`** ระบบใช้งานได้ครบทุกอย่าง แสดงอักษรย่อบนสีแบรนด์แทน
  · `banks.spec.ts` **อ่านโฟลเดอร์ `frontend/public/banks` จริง** แล้วตกทั้งสองทาง:
  ตารางอ้างไฟล์ที่ไม่มี และไฟล์ที่มีแต่ไม่มีใครอ้าง
- **⚠️ แก้ `banks.ts` แล้วต้อง `pnpm build:shared` เสมอ** เจอของจริงมาแล้ว: เพิ่มไฟล์ `071.jpg`
  และแก้ตารางครบแล้วแต่โลโก้ไม่ขึ้น เพราะทั้ง frontend และ backend import จาก
  `packages/shared/dist/` ไม่ใช่ `src/` · `dist/` อยู่ใน .gitignore และสร้างโดย `prepare` hook
  ตอน `pnpm install` เท่านั้น การแก้ไฟล์เฉยๆ ไม่ทำให้ dist เปลี่ยน
  · **ถ้ารันด้วย Docker ต้อง build image ใหม่ด้วย** เพราะทั้งไฟล์โลโก้และ dist ถูกคัดลอกเข้า image
  · README ใน `public/banks/` เขียนขั้นตอนนี้ไว้เป็นข้อ 3 พร้อมคำเตือนแล้ว
- **เทสต์ที่อ้างอิง "ธนาคารที่ยังไม่มีโลโก้" ห้ามระบุชื่อธนาคารจริง** ตอนแรกเขียนว่า
  `bankLogoPath('071')` ต้องเป็น null พอมีคนใส่ไฟล์ให้ 071 เทสต์ก็ตก ทั้งที่นั่นคือความสำเร็จ
  ไม่ใช่การถอยหลัง · เปลี่ยนเป็นทดสอบด้วยรหัสที่ไม่มีอยู่จริง (`999`) และวนตรวจทุกธนาคารว่า
  "มีไฟล์ → มี path · ไม่มีไฟล์ → เป็น null" ซึ่งจริงไม่ว่าโฟลเดอร์จะมีกี่ไฟล์
- **สีแบรนด์ธนาคารเป็นที่ที่สาม (และสุดท้าย) ที่ hardcode สีได้** ต่อจากอีเมล HTML และ `chart-theme.ts`
  เพราะเป็นแบรนด์ของคนอื่น ไม่ใช่ token ของดีไซน์ · ใช้เป็นพื้นของ badge fallback เท่านั้น
- **`BankLogo` วางทุกโลโก้ในกรอบขาวขนาดคงที่ + `object-contain`** เพื่อให้ `.jpg` ที่พื้นทึบ
  กับ `.gif` ที่โปร่งใส ดูเป็นชุดเดียวกัน และทุกอันมีขนาดเท่ากันไม่ว่าไฟล์ต้นทางจะสัดส่วนไหน
  · มี `onError` ตกไป badge ถ้าไฟล์หายไปภายหลัง · โปรเจกต์ไม่มี dark mode กรอบขาวจึงไม่หลุดบริบท
- **`BankSelect` เขียนเองด้วย `<button>` + `<input>` + `role="listbox"`** ไม่ใช้ไลบรารี (ข้อห้าม 12)
  และไม่ใช้ `<select>` เพราะแสดงโลโก้ไม่ได้ · ค้นได้ทั้งชื่อไทย ตัวย่อ และรหัส
  · **เจอจาก `test:responsive`: ช่องค้นหาสูง 20px เตี้ยเกินไปที่ 375px** แก้ด้วย `min-h-9`
- **`PayoutsService` ไม่แชร์โค้ดกับ `TopupsService` เลย ทั้งที่รูปทรงเกือบเหมือนกัน** (ข้อห้าม 16)
  เหตุผลที่หนักที่สุดไม่ใช่เรื่องรูปทรง แต่เพราะ **ADMIN อ่านเลขบัญชีได้ที่นี่ และต้องไม่มีทางอ่านได้ที่นั่น**
  ตารางเดียวกันแปลว่า select ชุดเดียวกันหลุดข้ามกันได้

### สิ่งที่ตัดสินใจเพิ่มตอนยกทั้งกองขึ้น Docker และซ้อมเส้นทางสาธิต

- **`MINIO_PUBLIC_ENDPOINT` / `_PORT` / `_USE_SSL` เป็นค่าบังคับ ไม่มีค่าเริ่มต้น** และ `StorageService`
  มี client สองตัว: `client` ใช้คุยกับ MinIO จากที่ที่ API รันอยู่ · `signingClient` ใช้เซ็น URL ที่**เบราว์เซอร์**จะเปิด
  ลายเซ็น S3 ครอบ Host ไปด้วย จึงแก้ทีหลังด้วยการ replace สตริงไม่ได้ ต้องเซ็นด้วย host ที่ถูกตั้งแต่แรก
  บนเครื่องพัฒนาสองค่านี้เป็น `localhost` เหมือนกันจนแยกไม่ออก — ใน `--profile full` API คุยกับ `minio`
  แต่เบราว์เซอร์คุยกับ `localhost` **ตั้งผิดจะไม่มีอะไรพังจนกว่าจะมีคนอัปโหลดไฟล์** จึงเลือกให้ fail ตอนสตาร์ต
- **`signingClient` ต้องถูก "บอก" region ไม่ใช่ปล่อยให้ไปถาม** — minio-js ที่ไม่ได้ตั้ง `region` จะยิงถาม
  bucket region ก่อนเซ็นทุกครั้ง ซึ่ง client ตัวนี้ทำไม่ได้เพราะ address ที่มันถืออยู่เป็นของเบราว์เซอร์
  ผลคือ `POST /uploads/presign` ตอบ 500 (`ECONNREFUSED 127.0.0.1:<พอร์ต MinIO>`) ทุกครั้งใน Docker
  วิธีแก้: ตั้ง `us-east-1` ไว้ก่อน แล้วให้ `onModuleInit` แก้ค่าจาก bucket จริงด้วย `client` ตัวที่ติดต่อได้
- **ห้ามใช้ `next/image` กับไฟล์จาก MinIO ทุกกรณี — ใช้ `<img>` เท่านั้น**
  `next/image` ไป optimize **ฝั่งเซิร์ฟเวอร์** คือตัว Next เองเป็นคนดึงรูป ไม่ใช่เบราว์เซอร์
  แต่ presigned URL ทุกใบถูกเซ็นด้วย host ที่**เบราว์เซอร์**เข้าถึงได้ สองฝั่งนี้คนละเครื่องกันใน Docker
  จึงขัดกันโดยธรรมชาติ ไม่ใช่เรื่องตั้งค่าผิด
  · อาการตอนเจอ: ปกไม่ขึ้นใน `--profile full` ทั้งที่ไฟล์ไม่มีปัญหา (เปิด presigned URL ตรงๆ ได้ 200
  ถอดรหัสเป็นภาพได้) เพราะ container `web` ไปดึง URL ที่ชี้ `localhost` ซึ่งใน container นั้น
  `localhost` คือตัวมันเอง (พิสูจน์แล้ว: ใน `web` ยิงพอร์ต MinIO บน `localhost` ได้ ECONNREFUSED
  · ยิง `minio:9000` ได้ 403) · **บนเครื่องพัฒนาไม่มีทางเจอ** เพราะ optimizer รันบน host
  ที่ `localhost:<พอร์ต MinIO>` คือ MinIO จริง
  บั๊กประเภทนี้จึงโผล่เฉพาะตอนสาธิต ซึ่งเป็นเวลาที่แย่ที่สุด
  · แก้แล้วทั้ง 6 ที่ที่แสดงปก (`CourseCard` · `CourseSearchBox` · หน้ารายละเอียดคอร์ส · `/my-courses`
  · `/home` · `GeneralTab` ของผู้สอน) ส่วนคิวอนุมัติของ ADMIN ใช้ `<img>` อยู่ก่อนแล้ว
  เช่นเดียวกับ QR สลิป และ avatar — **ตอนนี้ทั้งระบบไม่มี `next/image` เหลืออยู่เลยแม้แต่ที่เดียว**
  · **`fill` → `absolute inset-0 size-full object-cover`** เพราะ `fill` ก็คือ absolute inset-0 นั่นเอง
  กรอบทุกใบเป็น `relative aspect-video` (หรือ `size-11`) อยู่แล้ว เลย์เอาต์จึงไม่ขยับ (ตรวจซ้ำด้วย
  `pnpm test:responsive` แล้ว)
  · **ลบ `images.remotePatterns` ใน `next.config.ts` และ `NEXT_PUBLIC_MINIO_*` ทั้งสามตัวทิ้ง**
  (`.env.example` · `docker-compose.yml` · `frontend/Dockerfile`) มีไว้เพื่อ allow-list ให้ optimizer
  อย่างเดียว เก็บไว้จะกลายเป็นคำสั่งที่ชวนให้เข้าใจผิดว่ายังต้องตั้ง
- **`pnpm test:demo-flow`** ซ้อมเส้นทางสาธิต 20 ขั้นด้วยเบราว์เซอร์จริง ผู้ใช้สามคนสามหน้าต่าง
  ขั้น 7b และ 8 **เทียบตัวเลขจริง** ไม่ใช่แค่เปิดหน้าได้ (1,500.00 → 375.00 / 1,125.00 ต้องตรงกันทั้งสองหน้า)
  ใช้เวลา ~90 วินาที · เริ่มจากฐานข้อมูลที่มีแค่ ADMIN (`prisma/bootstrap-admin.ts`) · README อยู่ที่ `frontend/test/`
- **ไฟล์จำลองในสคริปต์เป็นไฟล์เล็กที่สุดที่ยอมรับได้** PNG 1x1 · MP4 ที่มีแต่ header · PDF หน้าเปล่า
  **MP4 นั้นเล่นไม่ได้จริง** หน้าห้องเรียนจึงขึ้น "เล่นวิดีโอไม่สำเร็จ" เป็นเรื่องปกติของ fixture ไม่ใช่บั๊ก
  การพิสูจน์วิดีโอจึงทำที่ระดับไบต์แทน: `GET /lessons/:id/stream` ตอบ 206 และไบต์ตรงกับที่อัปโหลดเป๊ะ
  ซึ่งพิสูจน์ข้อห้าม 10 ไปพร้อมกัน (ออกทาง API พอร์ต 4000 ไม่ใช่ presigned ของ MinIO)
- **ตัวเลือกของแบบทดสอบเป็น `<button aria-pressed>` ไม่ใช่ `<input type="radio">`** บันทึกไว้เพราะหลงหาผิดมาแล้ว
  (และเป็นจุดที่ควรพิจารณาเรื่อง screen reader ในอนาคต ยังไม่ได้แก้เพราะอยู่นอกขอบเขต ทก.01)
...
- **เกณฑ์ผ่านแบบทดสอบ (`Quiz.passScore`) ต้องอยู่ 50–100** — เดิมมี validation อยู่แล้วแต่พื้นล่างเป็น 1 ไม่ใช่ 50
  แก้แค่ backend เพราะตอนนั้น **หน้าจอให้ผู้สอนสร้าง/แก้ไขแบบทดสอบยังไม่มี**
  · **ปัจจุบันมีแล้ว** (`QuizzesTab.tsx` + `QuizForm.tsx` ดูหัวข้อ ทก.01 A7 ด้านบน) และพื้นล่าง
  **ขยับจาก 50 เป็น 60** ตาม ทก.01 · ค่าจริงอ่านจาก `QUIZ_PASS_SCORE_MIN` ใน `limits.ts` ไม่ได้ hardcode
...
- ~~**"ยอดค้างจ่ายผู้สอน" = รายได้สะสมทั้งหมด เพราะยังไม่มีการถอนเงิน"**~~
  **ไม่จริงแล้ว** ระบบถอนเงินทำแล้ว · `outstanding` หักยอดที่โอนออกจริงแล้ว และการ์ดบนรายงาน
  เปลี่ยนชื่อเป็น "รายได้ผู้สอน (สะสมในช่วง)" เพราะป้ายเดิมเรียกผิดมาตลอด — ดูหัวข้อระบบถอนเงินด้านบน
...
- ~~**ข้อจำกัด: ยังไม่ได้ตรวจ responsive บนจอ 375px และยังไม่ได้ลองติดตั้งบนเครื่องเปล่า**~~
  **ปิดทั้งคู่แล้ว** `pnpm test:responsive` ตรวจทุกหน้าที่ 375/768/1440 ด้วยเบราว์เซอร์จริงและรันซ้ำได้
  (เจอและแก้ตารางที่พัง 5 จุด) · และ clone ใหม่ลง path สั้นแล้ว build ผ่านครบทั้งชุด
...
- **ไม่มี migration ในเฟสนี้เลย** — `QnaThread` และ `QnaReply` มีใน schema ตั้งแต่เฟส 1 แล้ว
  · **การถอนเงินทำไปแล้วภายหลัง** (`PayoutRequest` + `InstructorBankAccount` — ดูหัวข้อระบบถอนเงินด้านบน)
  ส่วนรีวิวและการแจ้งเตือนยังไม่มี model
...
- **ข้อจำกัดที่รู้แล้ว: ยังไม่มีการแจ้งเตือน** ผู้สอนต้องเปิด `/instructor/qna` เอง ระบบไม่ส่งอีเมลหรือแจ้งในเว็บ
  · **และตอนนี้ไม่มีกระดิ่งบน header แล้วด้วย** (ถอดออกเพราะเป็นคำสัญญาที่ไม่มีของรองรับ — ดูหัวข้อด้านบน)
  ส่วนที่เหลือของเฟส 7 คือรีวิว ส่วนการถอนเงินทำไปแล้ว
...
- **แท็บ "ถาม-ตอบ" ตอนทำเฟส 6 เป็น empty state ที่บอกว่าจะเปิดใช้ในเฟส 7** ตามข้อห้าม 14
  หน้าบทเรียนออกแบบมาเป็นสามแท็บ การซ่อนไปเลยจะทำให้เลย์เอาต์เพี้ยนกว่าการบอกความจริง
  · **เฟส 7 เปลี่ยนเป็นลิงก์เข้ากระดานจริงแล้ว**
...
- ~~**ข้อจำกัด: ยังไม่มีหน้าจอให้ผู้สอนสร้างข้อสอบ**~~ **ทำแล้ว** — `QuizzesTab.tsx` แทนที่ empty state นั้น
  (ดูหัวข้อ ทก.01 A7 ด้านบน) · **นี่คือเคสแรกของ "หลังบ้านเสร็จแต่หน้าจอไม่เคยต่อ"** ที่ต่อมาเกิดซ้ำ
  กับปุ่มดูตัวอย่างบทเรียน — ทั้งสองเคสเป็นงานที่ `PLAN.md` วางไว้แต่ ทก.01 ไม่ได้กำหนด
  การไล่ตรวจตาม ทก.01 อย่างเดียวจึงมองไม่เห็น
...
- **แก้คอร์สได้ทุกสถานะยกเว้น `PENDING_REVIEW`** (`COURSE_NOT_EDITABLE` 409)
  **กติกา "แก้ราคาคอร์สที่เผยแพร่แล้วต้องส่งอนุมัติใหม่" ทำแล้ว** (`courses.service.ts` — `priceChanged`
  ดันสถานะกลับเป็น `PENDING_REVIEW`) ทำพร้อมคิวอนุมัติของ ADMIN ในเฟส 8 ตามที่ตั้งใจไว้
...
- โครงสร้างฐานข้อมูลจริงใช้ตามที่กำหนดในเฟส 1 ซึ่งกระชับกว่ารายการ entity ใน PLAN.md
  ...
  `Quiz` ผูกกับ Lesson ไม่ใช่ Course · ยังไม่มี Review, Certificate,
  Notification, PlatformSetting, AuditLog) ให้เพิ่มทีละส่วนตามเฟสที่ต้องใช้จริง
  · **`PayoutRequest` และ `InstructorBankAccount` เพิ่มเข้ามาภายหลังพร้อมระบบถอนเงิน**
...
- ผังบัญชีของจริงคือ `USER_WALLET` / `PLATFORM_REVENUE` / `EXTERNAL_BANK`
  ...
  · **เพิ่ม `PAYOUT` ใน `TxType` แล้วตอนทำระบบถอนเงิน พร้อมกับ `AccountKind.PAYOUT_PAYABLE` ที่เป็นบัญชีพัก**
